...
const SPEND = [
  ...
  {
    category: 'Eating out',
    payees: ['Barista', 'Burger Hut', 'The Curry House'],
    min: 700,
    max: 4_500,
    perMonth: 5,
  },
  {
    category: 'Transport',
    payees: ['PickMe', 'Fuel station', 'Bus pass'],
    min: 300,
    max: 6_000,
    perMonth: 6,
  },
  ...
  {
    category: 'Shopping',
    payees: ['Odel', 'Fashion Bug', 'Online order'],
    min: 2_000,
    max: 18_000,
    perMonth: 2,
  },
  {
    category: 'Household',
    payees: ['Hardware store', 'Cleaning service'],
    min: 1_000,
    max: 7_000,
    perMonth: 2,
  },
...
const accounts = transaction(() => ({
  bank: createAccount(user.id, {
    ...
    type: 'bank',
    openingBalance: rupees(42_000),
  }),
  wallet: createAccount(user.id, { name: 'Wallet', type: 'cash', openingBalance: rupees(3_500) }),
  savings: createAccount(user.id, {
    name: 'Savings',
    type: 'savings',
    openingBalance: rupees(180_000),
  }),
  card: createAccount(user.id, {
    name: 'Credit card',
    type: 'card',
    openingBalance: rupees(-12_000),
  }),
}));
...
transaction(() => {
  ...
  for (const [name, cap] of [
    ...
    upsertBudget(user.id, {
      categoryId: categories.get(name).id,
      month: thisMonth,
      amount: rupees(cap),
    });
  }
